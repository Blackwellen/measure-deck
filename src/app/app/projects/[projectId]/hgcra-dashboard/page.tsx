  ...
  for (const { app, timeline } of rows) {
    ...
    if (daysToPlnCutoff >= -14) {
      upcomingDates.push({
        ...
        type: daysToPlnCutoff < 0 ? "overdue" : "pln_cutoff",
        ...
        <div className="page-content space-y-6">
          ...
          <div className="card overflow-hidden">
            <div className="px-5 py-4 border-b border-[var(--border)] flex items-center justify-between">
              ...
              <span className="text-xs text-[var(--text-muted)]">{rows.length} application(s)</span>
            </div>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {rows.map(({ app, timeline, pln }) => {
                    const overdueDays = getDaysOverdue(timeline);
                    return (
                      <tr key={app.id}>
                        <td>
                          <Link
                        ...
                        <td>
                          <div className="flex items-center justify-end gap-1">
                            <Link
                              href={`/app/applications/${app.id}/pay-less-notice`}
                            ...
                            <Link
                              href={`/app/applications/${app.id}/suspension-notice`}
                              className="btn btn-ghost btn-sm text-xs"
              ...
              <div className="p-4 space-y-2">
                {upcomingDates.length === 0 ? (
                  ...
                  upcomingDates.map((ud, i) => {
                    ...
                    const isWarning = !isOverdue && !isCritical && ud.urgencyDays <= 5;
                    return (
                      <div
                        key={i}
                        className={cn(
              ...
              <div className="p-4">
                {suspensions.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {suspensions.map((s) => {
                      const appRow = rows.find((r) => r.app.id === s.application_id);
