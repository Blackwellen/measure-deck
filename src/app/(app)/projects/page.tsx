...
// ─── Open-change & next-app seed lookup ──────────────────────────────────────
...
const NEXT_APP: Record<string, string> = {
  "proj-001": "23 Jun", "proj-002": "14 Jul", "proj-003": "—",
  "proj-004": "30 Jun", "proj-005": "01 Jul", "proj-006": "—", "proj-007": "—",
};
...
function ProjectsPageInner() {
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "var(--text-muted)" }}>
            {activeProjects.filter(p => p.status === "active").length} active projects · {formatCurrency(totalValue)} total contract value
          </p>
        </div>
      ...
      {/* ─── Portfolio KPI strip (board view) ─────────────────────────────── */}
      {view === "board" && (
              ...
              return (
                <tr key={p.id} className="cursor-pointer" onClick={() => onRowClick(p.id)}>
                  ...
                  <td className="text-right tabular-nums">
                    <span className={cn(
                      "font-semibold",
                      p.margin >= 15 ? "text-[var(--success)]" :
                      p.margin >= 10 ? "text-[var(--warning)]" : "text-[var(--danger)]"
                    )}>
                      {p.margin.toFixed(1)}%
                    </span>
                  </td>
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            ...
            <p
              ...
              style={{
                color: p.margin >= 15 ? "var(--success)" :
                       p.margin >= 10 ? "var(--warning)" : "var(--danger)",
              }}
            >
              {p.margin >= 10
                ? <TrendingUp size={12} />
            ...
            >
              ...
              {items.length > 0 && (
                <span className="text-[10px]" style={{ color: "var(--text-muted)" }}>
                  £{(colValue / 1_000_000).toFixed(1)}m
                </span>
              )}
            </div>
            {/* Cards */}
