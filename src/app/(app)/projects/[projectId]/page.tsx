...
import {
  ...
  BarChart3, Building2, MapPin, Shield,
  ChevronRight, TrendingUp, TrendingDown, Eye, Link2,
  CheckCircle2, Circle, MoreHorizontal, Archive,
} from "lucide-react";
import {
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-value flex items-center gap-1">
            ...
            <span style={{ color: project.margin >= 15 ? "var(--success)" : "var(--warning)" }}>
            ...
            <tbody>
              {periods.map(p => (
                <tr key={p.id}>
                  ...
                  <td className="text-right tabular-nums font-bold" style={{ color: p.margin >= 15 ? "var(--success)" : "var(--warning)" }}>
                    {p.margin}%
                  </td>
    ...
    return (
      <div className="card">
        <div className="empty-state py-16">
          ...
          <h3 className="text-base font-semibold">No Site Plan Uploaded</h3>
          <p className="text-sm max-w-xs" style={{ color: "var(--text-muted)" }}>
            Upload a site plan to add location-based annotations, evidence pins, and zone markers.
          </p>
          <button className="btn btn-primary btn-sm mt-2" onClick={() => setHasPlan(true)}>
            <Upload size={14} />Upload Site Plan
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button className="btn btn-secondary btn-sm"><Plus size={13} />Add Zone</button>
        ...
        <button className="btn btn-secondary btn-sm ml-auto"><Download size={13} />Export</button>
      </div>
      <div
        className="card flex items-center justify-center"
        style={{ height: 480, background: "var(--bg-subtle)" }}
      >
        <p className="text-sm" style={{ color: "var(--text-muted)" }}>Site plan canvas</p>
      </div>
    </div>
  );
}
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="badge chip-primary text-[10px]">V1.5</span>
          </div>
        </div>
        <div className="flex items-center justify-center h-32 rounded-xl border-2 border-dashed" style={{ borderColor: "var(--border)" }}>
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>BIM model viewer — available in V1.5</p>
        </div>
      </div>
      ...
      <div className="card p-5">
        <div className="flex items-center gap-2 mb-4">
          <h3 className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>4D/5D Links</h3>
          <span className="badge chip-primary text-[10px]">V1.5</span>
        </div>
        <div className="flex items-center justify-center h-24 rounded-xl border-2 border-dashed" style={{ borderColor: "var(--border)" }}>
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>4D scheduling & 5D cost integration — available in V1.5</p>
        </div>
      </div>
    </div>
  );
