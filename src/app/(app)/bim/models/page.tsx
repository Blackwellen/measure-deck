...
function FlagGate({ onUpgrade }: { onUpgrade: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 text-center px-4">
      ...
      <div className="max-w-md">
        <h2 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>
          BIM Library — V1.5 Feature
        </h2>
        <p className="mt-2 text-sm leading-relaxed" style={{ color: "var(--text-secondary)" }}>
          Upload, manage and link IFC, Revit and NavisWorks models directly to your commercial records.
          Includes 3D preview, 4D schedule linking and 5D cost mapping.
        </p>
        <div className="mt-4 flex items-center gap-2 justify-center flex-wrap text-xs" style={{ color: "var(--text-muted)" }}>
          <span className="flex items-center gap-1"><Box size={12} />IFC / Revit / NavisWorks</span>
          <span>·</span>
          <span className="flex items-center gap-1"><Link2 size={12} />Link to Drawings</span>
          <span>·</span>
          <span className="flex items-center gap-1"><Calendar size={12} />4D Schedule</span>
          <span>·</span>
          <span className="flex items-center gap-1"><HardDrive size={12} />5D Cost Mapping</span>
        </div>
      </div>
      <div className="flex items-center gap-3">
        <button className="btn btn-primary" onClick={onUpgrade}>
          <Plus size={14} />Upgrade to V1.5
        </button>
        <button className="btn btn-secondary" onClick={() => window.open("https://measuredeck.com/bim", "_blank")}>
          Learn More
        </button>
      </div>
      <div
      ...
      >
        ...
        BIM module is not enabled on your current plan
      </div>
    </div>
  );
}
      ...
      >
        {model.status === "processing" ? (
          ...
          <div className="flex flex-col items-center gap-2 text-white">
            ...
            <span className="text-xs font-semibold opacity-60">3D Preview — V1.5</span>
          </div>
        )}
        {/* Type badge */}
        <div
        ...
        <div className="page-header">
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: "var(--text-muted)" }}>
              Manage IFC, Revit and NavisWorks models linked to your projects
            </p>
          </div>
        </div>
        <div className="page-content">
          <FlagGate onUpgrade={() => toast.info("Contact your account manager to enable BIM Module (V1.5)")} />
        </div>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map(model => (
              <ModelCard
                key={model.id}
                model={model}
                onView={() => router.push(`/bim/${model.id}`)}
